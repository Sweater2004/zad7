...
class CIBlockPropertyCProp
{
    ...
    private static function getControlName($strHTMLControlName)
    {
        return str_replace(':', '', $strHTMLControlName['VALUE']);
    }
    //Internals
    public static function ShowHtml($code, $title, $arValue, $strHTMLControlName)
    {
        $val = $arValue['VALUE'][$code];
        $name = self::getControlName($strHTMLControlName) . "[$code]";
        ...
        $html = preg_replace("/(\s)(name|for)=\"[^\"]+\"/", "$1$2=\"$name\"", $html);
        return '<tr>
